...
from apps.reservas.services import serializar_comanda_desde_reserva
...
def api_mesas_libres(request):
    ...
    mesas_bloqueadas_ids = set()
    reservas_conflictivas = Reserva.objects.filter(
        estado__in=['PENDIENTE', 'CONFIRMADA'],
        mesa__isnull=False
    )
    for r in reservas_conflictivas:
        if r.fecha and r.hora:
            ...
            if umbral_pasado <= dt_res <= limite_dt:
                mesas_bloqueadas_ids.add(r.mesa_id)

    qs = Mesa.objects.filter(
        estado=Mesa.Estado.LIBRE, 
        activo=True
    ).exclude(id__in=mesas_bloqueadas_ids).select_related('zona')
...
def api_estado_actual(request):
    ...
    for c in comandas_activas:
        ...
        comanda_data = {
            'id':              c.pk,
            'fecha_apertura':  timezone.localtime(c.fecha_apertura).strftime('%I:%M %p'),
    ...
    # 3. Reservas confirmadas pendientes de llegada (mesa RESERVADA sin comanda aún)
    reservas_pendientes = Reserva.objects.filter(
        mesa__isnull=False,
        estado='CONFIRMADA',
        pedido_confirmado=True,
        cliente_llego=False,
    ).prefetch_related('platos__plato', 'pago')
    reserva_por_mesa = {r.mesa_id: r for r in reservas_pendientes}

    # 4. Cargar todas las uniones activas
    ...
    # 5. Construir la respuesta para todas las mesas
    for mesa in Mesa.objects.filter(activo=True).select_related('zona').all():
        comanda_data = mesa_id_to_comanda.get(mesa.id)
        if not comanda_data and mesa.estado == Mesa.Estado.RESERVADA:
            reserva = reserva_por_mesa.get(mesa.id)
            if reserva:
                comanda_data = serializar_comanda_desde_reserva(reserva)
    ...
    # 6. Obtener IDs de platos inactivos o sin disponibilidad
