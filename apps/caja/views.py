...
from .services import procesar_cobro, registrar_perdida, finalizar_comanda_reserva_prepagada
...
def cobrar_view(request):
    ...
    reservas_prepagadas = {
        ...
        for r in Reserva.objects.filter(
            comanda__in=[c.id for c in comandas_listas],
            pago__estado='PAGADO',
        ).select_related('pago')
    }
    ...
    for c in comandas_listas:
        reserva_prepagada = reservas_prepagadas.get(c.id)
        c.reserva_prepagada = bool(reserva_prepagada)
        c.metodo_prepago = reserva_prepagada.pago.get_metodo_display() if reserva_prepagada else ''
...
@api_view(['POST'])
@permission_classes([EsCajeroOAdmin])
def api_finalizar_reserva_prepagada(request, pk):
    """Cierra una comanda pagada online al reservar y emite la boleta de venta."""
    try:
        pago = finalizar_comanda_reserva_prepagada(pk, request.user)
        return Response({
            'ok': True,
            'message': 'Comanda finalizada. Boleta generada.',
            'pago_id': pago.id,
            'pago_ids': [pago.id],
            'boleta_url': f'/caja/boleta/{pago.id}/',
        })
    except (ValidationError, Exception) as e:
        return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
